...
Latest commit: Friday, August 7, 2026, [**`×××××××`**](https://www.github.com/Zushah/WasmGPU/commit/HEAD).

Parent commit: Wednesday, August 5, 2026, [**`780432d`**](https://www.github.com/Zushah/WasmGPU/commit/780432d).
...
The CI selection table below makes the platform decision explicit instead of treating the eighteen-entry operating-system, architecture, and browser product as uniformly useful. A row is included only when GitHub supplies the host, Playwright supplies a native browser for that architecture, WebGPU is expected to exercise WasmGPU rather than fail at browser setup, and the row adds material coverage. Chromium supplies the portable operating-system and CPU-architecture baseline; Firefox is retained on its supported Windows and Apple Silicon targets; WebKit is retained on Apple Silicon as the closest automated Safari-family target. Linux Firefox remains outside CI while its WebGPU implementation requires manual enablement and exhibits external backend failures, Linux WebKit currently lacks `navigator.gpu`, and Windows ARM64 remains outside CI because Playwright currently supplies x64 rather than native ARM64 Windows browser packages. GitHub's `windows-2025` AMD64 image is the available Windows Server 2025 surrogate for the Windows 11 rows rather than a literal Windows 11 client installation.
