...
var<workgroup> tile_a: array<u32, 256>;
var<workgroup> tile_b: array<u32, 256>;
...
fn main(@builtin(workgroup_id) wg: vec3<u32>, @builtin(local_invocation_id) lid: vec3<u32>) {
    ...
    for (var base = 0u; base < k; base = base + TILE) {
        let ak = base + lid.x;
        let bk = base + lid.y;
        tile_a[local] = select(0u, a[row * k + ak], row < m && ak < k);
        tile_b[local] = select(0u, b[bk * n + col], bk < k && col < n);
        workgroupBarrier();
    ...
    if (row < m && col < n) {
        let beta = params[4];
        let old = select(0u, c[row * n + col], beta != 0u);
        c[row * n + col] = params[3] * acc + beta * old;
    }
}
