# compute.kernels.subC64

## Summary
compute.kernels.subC64 subtracts complex64 vectors element by element, with each value stored as interleaved `[real, imaginary]` `f32` components. It supports prefix counts, a distinct reusable output, and external command recording; otherwise it returns a new readback-capable storage buffer.

## Syntax
```ts
WasmGPU.compute.kernels.subC64(a: StorageBuffer, b: StorageBuffer, opts?: VectorKernelOptions): StorageBuffer
```
...
const out = wgpu.compute.kernels.subC64(a, b);
const values = await wgpu.compute.readback.readF32(out);
console.log(Array.from(values));
```

## See Also
- [compute.kernels.subF32](./compute-kernels-subf32.md)
