# compute.kernels.addU32

## Summary
compute.kernels.addU32 adds two `u32` vectors element by element with arithmetic wrapping modulo 2³². It can process a selected prefix, reuse a distinct caller-provided output, or record into an external encoder; otherwise it returns a new readback-capable storage buffer.

## Syntax
```ts
WasmGPU.compute.kernels.addU32(a: StorageBuffer, b: StorageBuffer, opts?: VectorKernelOptions): StorageBuffer
```
...
const out = wgpu.compute.kernels.addU32(a, b);
const values = await wgpu.compute.readback.readU32(out);
console.log(Array.from(values));
```

## See Also
- [compute.kernels.addF32](./compute-kernels-addf32.md)
- [compute.kernels.addC64](./compute-kernels-addc64.md)
