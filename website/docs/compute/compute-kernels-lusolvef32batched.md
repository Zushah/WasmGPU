# compute.kernels.luSolveF32Batched

## Summary
compute.kernels.luSolveF32Batched solves batched linear systems `A x = b` from compact LU factors and pivot rows produced by [compute.kernels.luFactorF32Batched](./compute-kernels-lufactorf32batched.md).
...
The multi-stage solve writes into caller-provided output storage and submits its own commands.
