# compute.kernels.addC64

## Summary
compute.kernels.addC64 adds complex64 vectors element by element, with each value stored as interleaved `[real, imaginary]` `f32` components. `opts.count` can restrict the operation, `opts.out` can reuse caller storage, and `opts.encoder` records the dispatch without submitting it; otherwise the method returns a new readback-capable output buffer.
...
- [compute.kernels.addU32](./compute-kernels-addu32.md)
