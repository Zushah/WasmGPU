# compute.kernels.sclF32

## Summary
compute.kernels.sclF32 multiplies every selected `f32` input element by one scalar. It can reuse a distinct output or record into an external encoder; otherwise it returns a new readback-capable storage buffer.
