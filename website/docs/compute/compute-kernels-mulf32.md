# compute.kernels.mulF32

## Summary
compute.kernels.mulF32 multiplies two `f32` vectors element by element. It supports prefix counts, a distinct reusable output, and external command recording; otherwise it returns a new readback-capable storage buffer.

## Syntax
```ts
WasmGPU.compute.kernels.mulF32(a: StorageBuffer, b: StorageBuffer, opts?: VectorKernelOptions): StorageBuffer
```
...
const out = wgpu.compute.kernels.mulF32(a, b);
const values = await wgpu.compute.readback.readF32(out);
console.log(Array.from(values));
```

## See Also
- [compute.kernels.mulU32](./compute-kernels-mulu32.md)
