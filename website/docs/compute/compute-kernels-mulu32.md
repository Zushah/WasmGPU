# compute.kernels.mulU32

## Summary
compute.kernels.mulU32 multiplies two `u32` vectors element by element with results wrapping modulo 2³². It supports prefix counts, a distinct reusable output, and external command recording; otherwise it returns a new readback-capable storage buffer.

## Syntax
```ts
WasmGPU.compute.kernels.mulU32(a: StorageBuffer, b: StorageBuffer, opts?: VectorKernelOptions): StorageBuffer
```
...
const out = wgpu.compute.kernels.mulU32(a, b);
const values = await wgpu.compute.readback.readU32(out);
console.log(Array.from(values));
```

## See Also
- [compute.kernels.mulF32](./compute-kernels-mulf32.md)
- [compute.kernels.mulC64](./compute-kernels-mulc64.md)
