# compute.kernels.gemmC64

## Summary
compute.kernels.gemmC64 computes row-major `out = alpha * A * B + beta * out` for complex64 matrices stored as interleaved `[real, imaginary]` `f32` pairs. Complex `alpha` and `beta` scale the product and existing destination; callers may reuse a distinct output or record the operation into an external encoder, while an omitted output is newly allocated and readback-capable.
...
- [compute.kernels.gemmU32](./compute-kernels-gemmu32.md)
