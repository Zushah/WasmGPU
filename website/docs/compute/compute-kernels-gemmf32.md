# compute.kernels.gemmF32

## Summary
compute.kernels.gemmF32 computes row-major `out = alpha * A * B + beta * out` for `f32` matrices. Callers may scale the product and existing destination, reuse a distinct output buffer, or record into an external encoder; an omitted output is newly allocated and readback-capable.
...
- [compute.kernels.gemmC64](./compute-kernels-gemmc64.md)
