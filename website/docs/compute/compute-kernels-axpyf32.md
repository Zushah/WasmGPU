# compute.kernels.axpyF32

## Summary
compute.kernels.axpyF32 computes `alpha * x + y` element by element for `f32` vectors. The operation can cover a selected prefix, reuse a distinct output buffer, or be recorded into a caller encoder; with no output supplied it returns a new readback-capable storage buffer.
