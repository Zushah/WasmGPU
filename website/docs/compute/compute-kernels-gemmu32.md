# compute.kernels.gemmU32

## Summary
compute.kernels.gemmU32 computes row-major `out = alpha * A * B + beta * out` with `u32` arithmetic wrapping modulo 2³². Callers may reuse a distinct output buffer or record into an external encoder; an omitted output is newly allocated and readback-capable.
