# material.custom#setResource

## Summary
material.custom#setResource replaces a resource at an existing binding and invalidates the material bind group without changing its pipeline layout.
...
The binding number must already exist in the immutable layout. The replacement is used on subsequent draws. Buffer range records are snapshotted, while their WebGPU resources remain caller-owned.
...
- [webgpu.bindingResource](../interop/webgpu-bindingresource.md)
