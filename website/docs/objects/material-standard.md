...
## Summary
material.standard creates a physically based surface material for lit meshes. Use it for the core metallic-roughness workflow, optional texture transforms, and the glTF-oriented extension blocks that cover clearcoat, transmission, sheen, iridescence, anisotropy, and related effects.
...
Use `transmission` for specular transmission and `diffuseTransmission` for tinted diffuse-through-surface behavior. Either one makes the material sample rendered scene color when visible. `volume` adds thickness and attenuation controls for transmitted light, and `dispersion` adds a dispersion scalar on top of that path. This is a practical real-time transmission model, not a claim of physically exact refraction.
...
## See Also
- [material.standard#getLayoutPlan](./material-standard-getlayoutplan.md)
