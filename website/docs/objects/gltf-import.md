...
## Summary
gltf.import converts a loaded `GltfDocument` into WasmGPU runtime resources. It builds meshes, materials, textures, optional cameras and punctual lights, imported-node wrappers, skins, animation clips, and import metadata in one pass.
...
| --- | --- | --- | --- |
| `doc` | `GltfDocument` | Yes | Loaded glTF document from [gltf.load](./gltf-load.md) or an equivalent source. |
...
| `addToScene` | `boolean` | No | When true or omitted, imported meshes, splat fields, and lights are inserted into the destination scene. Set this to `false` if you want detached runtime objects first. Cameras remain available through the result and imported nodes rather than becoming scene members. |
