...
## Summary
texture.create2D constructs a `Texture2D` wrapper that defers GPU upload until the texture is first used or explicitly uploaded. Use it for manually authored textures and as the runtime texture container that glTF import maps images and sampler state into.
