# gltf.readAccessorAsFloat32

## Summary
gltf.readAccessorAsFloat32 reads one accessor as compact `Float32Array` components. Unnormalized float32 data returns the raw reader's array and can therefore alias a document buffer; other component types allocate a new array. Normalized integer accessors are converted to their glTF normalized floating-point range.
...
## Returns
`Float32Array` - Accessor payload converted to float32 elements.
