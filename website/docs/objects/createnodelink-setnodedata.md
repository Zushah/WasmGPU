...
WebAssembly channel setters borrow memory and refresh immediately. Their matching refresh methods re-read the source after producer writes or memory growth. Wasm positions, colors, and radii are fixed `vec4` records; Wasm scalars use one float per node. Options control `nodeCount`, grow-only managed GPU record capacity, CPU retention, and position-only bounds recomputation. Passing `null` detaches that channel and destroys its managed GPU copy.

CPU setters copy/pack their input. Update methods also copy queued GPU patch bytes and update retained CPU arrays only when present. External buffers must support storage binding and have sufficient capacity; those properties are caller-enforced. Replacing an owned buffer destroys it, while a borrowed replacement is only detached.
