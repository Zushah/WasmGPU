# createNodeLink#decodePickElement

This page documents the `NodeLink.decodePickElement` method.

## Summary
`createNodeLink#decodePickElement` maps a picking element index back to either a node or an edge. Element indices map to nodes first, then to edges.
...
| --- | --- | --- | --- |
| `elementIndex` | `number` | Yes | Picking element index to decode back into node or edge space. |
