...
## Summary
WasmGPU.gltf.readAccessorAsFloat32 reads one accessor from a loaded glTF document and returns it as `Float32Array`. Native float32 accessors are returned directly when no conversion is needed; other numeric component types are converted through the WebAssembly accessor helpers.
