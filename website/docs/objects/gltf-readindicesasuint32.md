# gltf.readIndicesAsUint32

## Summary
gltf.readIndicesAsUint32 reads one valid glTF index accessor as a compact `Uint32Array`. The accessor must be `SCALAR`, use an unsigned integer component type, and not be normalized. Native uint32 data may alias a document buffer; other unsigned component types allocate a converted array. For complete primitive validation, prefer the importer.
