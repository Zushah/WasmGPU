# createPointCloud#getLocalBounds

## Summary
createPointCloud#getLocalBounds returns explicit or computed bounds in object-local space. When no bounds exist but CPU point records are retained, it computes them lazily. Otherwise it returns empty bounds; `partial` is `true` when points exist but their positions are unavailable on the CPU.
