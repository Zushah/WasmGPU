...
## Summary
gltf.readAccessor reads one accessor from a loaded glTF document and returns its metadata plus the raw typed-array payload. Compact, aligned, non-sparse accessors may return a view that aliases the document buffer; deinterleaved, matrix-padding, missing-bufferView, or sparse paths allocate a new compact array. Sparse patches are applied to a copy. Integer normalization is reported but not applied by this raw reader.
...
| `array` | `GltfTypedArray` | Yes | Compact raw components. It may alias a document buffer when no compaction or sparse patch is required. |
