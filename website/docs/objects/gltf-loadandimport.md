...
## Summary
gltf.loadAndImport combines [gltf.load](./gltf-load.md) and [gltf.import](./gltf-import.md) into one call. It loads a `.gltf` or `.glb` source, resolves buffers and optional image payloads, then converts the document into WasmGPU scene resources and import metadata.
...
For the full `LoadGltfOptions`, `ImportGltfOptions`, `GltfImportedNode`, `GltfImportMetadata`, material-variant controls, and extension-support details, use the dedicated [gltf.load](./gltf-load.md) and [gltf.import](./gltf-import.md) pages.
