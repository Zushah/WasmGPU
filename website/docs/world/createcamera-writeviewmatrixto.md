# createCamera#writeViewMatrixTo

## Summary
createCamera#writeViewMatrixTo writes the camera's f32 view matrix directly into the active WasmGPU transform store at a byte pointer.
The caller must provide a four-byte-aligned pointer to at least 64 writable bytes in the current driver memory; the 16 column-major values transform world space into view space.
