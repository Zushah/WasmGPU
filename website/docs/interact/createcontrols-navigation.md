# createControls.navigation

## Summary
createControls.navigation creates a `NavigationControls` instance that can operate in orbit, trackball, or fly mode.
...
The keyboard, pointer-lock, and movement fields apply while `mode` is `"fly"`. Their defaults and key bindings are listed on [createControls.fly](./createcontrols-fly.md).
