# createControls.trackball

## Summary
createControls.trackball creates `TrackballControls`, a `NavigationControls` specialization that starts in free-form trackball mode.
...
The factory overrides the descriptor's initial `mode` with `"trackball"`. The returned controller still inherits `setMode()` and can switch to orbit or fly behavior later. See [createControls.navigation](./createcontrols-navigation.md) for the complete descriptor.
...
## See Also
- [createControls.navigation](./createcontrols-navigation.md)
