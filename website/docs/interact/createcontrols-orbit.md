...
## Summary
createControls.orbit creates `OrbitControls`, a `NavigationControls` specialization that starts in orbit mode.
...
The factory overrides the descriptor's initial `mode` with `"orbit"`. The returned controller still inherits `setMode()` and can switch to trackball or fly behavior later. See [createControls.navigation](./createcontrols-navigation.md) for the complete descriptor.
