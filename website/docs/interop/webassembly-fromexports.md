# webassembly.fromExports

## Summary
webassembly.fromExports wraps a raw exports object and returns a `WasmModule`.
...
- `WasmMemoryView` recreates cached typed views automatically after memory growth. Call `refresh()` when an exported pointer, length, byte offset, dtype, or memory selection may have changed.
