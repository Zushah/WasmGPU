# webassembly.fromMemory

## Summary
webassembly.fromMemory wraps a bare `WebAssembly.Memory` and returns a `WasmModule`.
...
- Because this wrapper is built from memory alone, it does not provide functions or globals unless you wrap an exports object through [webassembly.fromExports](./webassembly-fromexports.md) or an instance through [webassembly.fromInstance](./webassembly-frominstance.md) instead.
...
- `WasmMemoryView` recreates its cached typed views automatically when memory growth replaces `memory.buffer`. Call `refresh()` only when descriptor-resolved metadata may have changed; this memory-only example uses fixed pointer and length values.
