# math.vec3.isOrthogonal

## Summary
math.vec3.isOrthogonal checks whether the dot product is exactly zero. It does not apply an epsilon tolerance; consequently, a zero vector is considered orthogonal to every vector.
...
```ts
WasmGPU.math.vec3f.isOrthogonal(a: WasmPtr, b: WasmPtr): boolean
WasmGPU.math.vec3d.isOrthogonal(a: WasmPtr, b: WasmPtr): boolean
```
