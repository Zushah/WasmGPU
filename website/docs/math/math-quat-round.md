...
## Summary
math.quat.round rounds each component to the nearest integer. It is useful when snapping values to a grid or discrete representation.

## Syntax
```ts
WasmGPU.math.quat.round(q: number[]): number[]
const result = wgpu.math.quat.round(q);
```

## Precision-Specific Wasm Forms
```ts
WasmGPU.math.quatf.round(out: WasmPtr, q: WasmPtr): void
WasmGPU.math.quatd.round(out: WasmPtr, q: WasmPtr): void
```
