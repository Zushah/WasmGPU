# WasmGPU.math.vec3.round

## Summary
WasmGPU.math.vec3.round rounds each component to the nearest integer. It is useful when snapping values to a grid or discrete representation.

## Syntax
```ts
WasmGPU.math.vec3.round(v: number[]): number[]
const result = wgpu.math.vec3.round(v);
```

## Precision-Specific Wasm Forms
```ts
WasmGPU.math.vec3f.round(out: WasmPtr, v: WasmPtr): void
WasmGPU.math.vec3d.round(out: WasmPtr, v: WasmPtr): void
```
