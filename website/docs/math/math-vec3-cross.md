...
## Summary
math.vec3.cross computes the 3D cross product of two vectors. The result is orthogonal to both inputs and oriented by right-hand rule.
...
```ts
WasmGPU.math.vec3f.cross(out: WasmPtr, a: WasmPtr, b: WasmPtr): void
WasmGPU.math.vec3d.cross(out: WasmPtr, a: WasmPtr, b: WasmPtr): void
```
