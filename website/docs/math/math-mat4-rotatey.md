...
## Summary
math.mat4.rotateY applies rotation around the Y axis by a radian angle. Use it for yaw-like rotations around the vertical axis.
...
```ts
WasmGPU.math.mat4f.rotateY(out: WasmPtr, m: WasmPtr, angle: number): void
WasmGPU.math.mat4d.rotateY(out: WasmPtr, m: WasmPtr, angle: number): void
```
...
| --- | --- | --- | --- |
| `matr` | `number[]` | Yes | Column-major 4x4 matrix to rotate about the Y axis. |
