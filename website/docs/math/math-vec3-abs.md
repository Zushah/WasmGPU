...
## Summary
math.vec3.abs returns the component-wise absolute value of the input. Use it to remove sign from each component without changing magnitude ordering.
...
```ts
WasmGPU.math.vec3f.abs(out: WasmPtr, v: WasmPtr): void
WasmGPU.math.vec3d.abs(out: WasmPtr, v: WasmPtr): void
```
