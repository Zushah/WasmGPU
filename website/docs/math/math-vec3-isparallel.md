# math.vec3.isParallel

## Summary
math.vec3.isParallel checks exact same-direction parallelism by testing `dot(v1, v2) === length(v1) * length(v2)`. Opposite directions return `false`, and a zero vector is considered parallel to every vector. No epsilon tolerance is applied.
...
```ts
WasmGPU.math.vec3f.isParallel(a: WasmPtr, b: WasmPtr): boolean
WasmGPU.math.vec3d.isParallel(a: WasmPtr, b: WasmPtr): boolean
```
