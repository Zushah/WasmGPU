...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    let j1 = in.joints1;
    let w1 = in.weights1;
    let skin_matrix = skin.joints[j0.x] * w0.x +
    ...
    let local_pos = skin_matrix * vec4<f32>(in.position, 1.0);
    let local_normal = (skin_matrix * vec4<f32>(in.normal, 0.0)).xyz;
    let local_tangent = (skin_matrix * vec4<f32>(in.tangent.xyz, 0.0)).xyz;
    let world_pos4 = model.model * local_pos;
    out.position = camera.view_projection * world_pos4;
    out.world_pos = world_pos4.xyz;
    out.normal = normalize((model.normal_matrix * vec4<f32>(local_normal, 0.0)).xyz);
    out.tangent = vec4<f32>(
        (model.normal_matrix * vec4<f32>(local_tangent, 0.0)).xyz,
        in.tangent.w,
    );
    out.model_scale = vec3<f32>(
        length(model.model[0].xyz),
        length(model.model[1].xyz),
        length(model.model[2].xyz),
    );
    out.uv = in.uv;
    out.uv1 = in.uv1;
    out.color = in.color;
