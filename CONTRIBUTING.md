...
5. Build the JavaScript bundles and run tests with `npm run build` and `npm run test`, or simply `npm run dev`. Note that `npm run test:js` launches isolated Chromium pages with WebGPU enabled and deterministic SwiftShader GPU execution. Use `npm run test:js:headed` or `npm run test:js:debug` when diagnosing browser failures. Make sure all tests pass. Update current tests or add new tests if applicable.
...
7. Use `npm run restore` to restore the built JavaScript bundles of the latest release, since new bundles are only committed for new releases.
8. Submit a [pull request](https://www.github.com/Zushah/WasmGPU/pulls) against the `main` branch of the `Zushah/WasmGPU` repository.
