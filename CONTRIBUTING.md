...
3. Install development dependencies with `npm install`, then install the pinned Playwright Chromium build with `npx playwright install --with-deps --no-shell chromium`. These commands install [TypeScript](https://www.npmjs.com/package/typescript), [Node types](https://www.npmjs.com/package/@types/node), [WebGPU types](https://www.npmjs.com/package/@webgpu/types), [esbuild](https://www.npmjs.com/package/esbuild), [Playwright](https://www.npmjs.com/package/playwright), and you may also need to install additional development dependencies depending on where exactly you want to contribute to the codebase. Note that WasmGPU has zero runtime dependencies and it is very highly preferable to keep it that way.
