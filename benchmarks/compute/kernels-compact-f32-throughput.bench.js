...
export default {
    name: "kernels-compact-f32-throughput",
    subsystem: "compute",
    type: "throughput",
    unit: "elements/s",
    description: "Complete ComputeKernels.compactF32 throughput at 50% keep density, excluding readback and including GPU completion.",
    ...
    run(state, { compute }) {
        const result = compute.kernels.compactF32(
            state.input,
            state.flags,
            { out: state.output }
        );
