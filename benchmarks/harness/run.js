...
const artifactMarker = tracked ? ".bench" : ".bench.local";
...
try {
    ...
    await page.exposeFunction("__wasmgpuBenchmarkProgress", ({ index, subsystem, name, elapsedMilliseconds, devices, submissions }) => {
        console.log(`${green}[bench:${String(index).padStart(2, "0")}:${subsystem}/${name}] done in ${Math.round(elapsedMilliseconds)}ms, devices=${devices}, submissions=${submissions}${reset}`);
    });
    ...
    writeFileSync(resolve(logsDir, `run${artifactMarker}.json`), `${JSON.stringify(manifest)}\n`);
    for (const result of output.results) writeFileSync(resolve(logsDir, `${result.subsystem}-${result.name}${artifactMarker}.json`), `${JSON.stringify(result)}\n`);
