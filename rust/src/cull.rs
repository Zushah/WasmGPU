...
fn write_all_visible(out: &mut [u32], centers: &[f32], radii: &[f32], out_stats: Option<&mut [u32]>) -> u32 {
    let n = radii.len();
...
#[no_mangle]
pub extern "C" fn cull_spheres_occlusion(out_indices_ptr: u32, out_stats_ptr: u32, centers_ptr: u32, radii_ptr: u32, count: u32, view_proj_ptr: u32, viewport_width: f32, viewport_height: f32, mip_offsets_ptr: u32, mip_widths_ptr: u32, mip_heights_ptr: u32, mip_count: u32, depth_values_ptr: u32, depth_values_len: u32, near_plane_epsilon: f32, max_screen_coverage: f32, depth_bias: f32) -> u32 {
    unsafe {
        ...
        let radii = f32_slice(radii_ptr, n);
        let out = u32_slice_mut(out_indices_ptr, n);
        let out_stats = if out_stats_ptr != 0 { Some(u32_slice_mut(out_stats_ptr, 3)) } else { None };
        ...
        for i in 0..n {
            ...
            let near_dist = (near_plane[0] * cx) + (near_plane[1] * cy) + (near_plane[2] * cz) + near_plane[3];
            ...
            for sx in [-1.0f32, 1.0f32] {
                for sy in [-1.0f32, 1.0f32] {
                    for sz in [-1.0f32, 1.0f32] {
                        ...
                        if !clip[0].is_finite() || !clip[1].is_finite() || !clip[2].is_finite() || !clip[3].is_finite() || clip[3] <= 1e-6 {
                            ambiguous = true;
                            break;
                        }
            ...
            let rect_min_y = (((1.0 - (max_y + half_pixel_y).min(1.0)) * 0.5)).max(0.0) * viewport_height;
            let rect_max_y = (((1.0 - (min_y - half_pixel_y).max(-1.0)) * 0.5)).min(1.0) * viewport_height;
            ...
            let mip_level = max_dim.log2().floor().max(0.0).min((mip_count_usize - 1) as f32) as usize;
